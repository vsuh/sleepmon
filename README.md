...
- Android использует **только secure RFCOMM/SPP**. Старый insecure fallback удалён.
...
`steps_total` хранится как отдельное актуальное поле. При разборе старых заметок backend умеет использовать legacy `steps_1 + steps_2`, если `steps_total` отсутствует.
...
- вручную изменить машинные показатели;
- задать `well_being`;
- указать `alco`;
...
Android использует отдельный `/sync` и не должен затирать ручные поля.
...
Сборку APK выполняет пользователь **локально**.
...
**В текущем рабочем окружении проекта APK не собирается.**
...
При расхождении между старой исторической документацией и текущим кодом ориентироваться на текущее состояние кода и обновлять `task.md`.
